-- yay Lua hooks (yay v13+). Loaded automatically from ~/.config/yay/init.lua.
--
-- Warn when an AUR package's maintainer changes between upgrades.
-- Source: https://github.com/Jguer/yay doc/examples/maintainer_change.lua
--
-- The known maintainer for each package is stored in a plain text cache file
-- inside the yay cache directory. On the first upgrade for a package the
-- current maintainer is recorded without any warning. On subsequent upgrades:
